...
from acquisition_model.utils import get_entropy, get_confidence, ind_to_onehot
...
class CMIEstimator(pl.LightningModule):
    ...
    def _safe_add_scalar(self, tag, value):
        ...
        if callable(add_scalar):
            add_scalar(tag, value, self.current_epoch)
    ...
    def on_train_batch_end(self, outputs, batch, batch_idx):
        if not isinstance(outputs, dict):
            return
        if 'predictor_loss' not in outputs or 'value_network_loss' not in outputs:
            return
        self._train_epoch_outputs.append({
            'predictor_loss': outputs['predictor_loss'].detach(),
            'value_network_loss': outputs['value_network_loss'].detach(),
        })

    def on_train_epoch_end(self):
        if not self._train_epoch_outputs:
            return
        pred_loss = torch.stack([out['predictor_loss'] for out in self._train_epoch_outputs]).mean()
        value_network_loss = torch.stack([out['value_network_loss'] for out in self._train_epoch_outputs]).mean()
        ...
        self._safe_add_scalar('Loss Train/Mean', pred_loss)
        self._safe_add_scalar('Value Loss Train/Mean', value_network_loss)
    ...
    def on_validation_epoch_end(self):
        ...
        self._safe_add_scalar('Loss Val/Mean', pred_loss_mean)
        self._safe_add_scalar('Perf Val/Mean', val_loss_mean)
        self._safe_add_scalar('Loss Val/Final', pred_loss_final)
        self._safe_add_scalar('Perf Val/Final', val_loss_final)
        self._safe_add_scalar('Eps Value', self.eps)
